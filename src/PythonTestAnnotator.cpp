...
public:
  ...
  TyErrorId process(CAS &tcas, ResultSpecification const &res_spec)
  {
    ...
    // boost numpy
    // python::tuple shape = python::make_tuple(100, 100);
    // np::ndarray arr = np::zeros(shape, np::dtype::get_builtin<float>());
    // np::ndarray res_arr = python::extract<np::ndarray>(rs_test_random_func(arr));
    // int nd = res_arr.get_nd();
    // outInfo("n dimension: " << nd);
